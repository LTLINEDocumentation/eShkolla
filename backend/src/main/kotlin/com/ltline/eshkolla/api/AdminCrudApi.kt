...
@Serializable data class AdminAbsenceUpdate(val studentId:String,val subjectId:String,val teacherId:String,val date:String,val status:String,val note:String?=null)
@Serializable data class AdminAbsenceDto(val id:String,val studentId:String,val subjectId:String,val teacherId:String,val date:String,val status:String,val note:String?=null)
...
fun Application.configureAdminCrudApi(authService:AuthService){
 routing{route("/api/v1/management"){
  ...
  get("/absences"){if(!adminOnly(call,authService))return@get;call.respond(loadAdminAbsences())}
  post("/absences"){if(!adminOnly(call,authService))return@post;val r=call.receive<AdminAbsenceUpdate>();validateAdminAbsence(r)?.let{call.respond(HttpStatusCode.BadRequest,ApiError("VALIDATION_ERROR",it));return@post};val id="ABS-"+UUID.randomUUID().toString().take(8).uppercase();Database.connection().use{db->db.prepareStatement("INSERT INTO absences(id,student_id,subject_id,teacher_id,date,status,note) VALUES(?,?,?,?,?,?,?)").use{p->p.setString(1,id);p.setString(2,r.studentId);p.setString(3,r.subjectId);p.setString(4,r.teacherId);p.setString(5,r.date);p.setString(6,r.status);p.setString(7,r.note?.takeIf{it.isNotBlank()});p.executeUpdate()}};call.respond(HttpStatusCode.Created,AdminAbsenceDto(id,r.studentId,r.subjectId,r.teacherId,r.date,r.status,r.note))}
  put("/absences/{id}"){if(!adminOnly(call,authService))return@put;val id=call.parameters["id"].orEmpty();val r=call.receive<AdminAbsenceUpdate>();validateAdminAbsence(r)?.let{call.respond(HttpStatusCode.BadRequest,ApiError("VALIDATION_ERROR",it));return@put};val n=Database.connection().use{db->db.prepareStatement("UPDATE absences SET student_id=?,subject_id=?,teacher_id=?,date=?,status=?,note=? WHERE id=?").use{p->p.setString(1,r.studentId);p.setString(2,r.subjectId);p.setString(3,r.teacherId);p.setString(4,r.date);p.setString(5,r.status);p.setString(6,r.note?.takeIf{it.isNotBlank()});p.setString(7,id);p.executeUpdate()}};if(n==0)call.respond(HttpStatusCode.NotFound,ApiError("NOT_FOUND","Mungesa nuk u gjet."))else call.respond(AdminAbsenceDto(id,r.studentId,r.subjectId,r.teacherId,r.date,r.status,r.note))}
  delete("/absences/{id}"){if(!adminOnly(call,authService))return@delete;val id=call.parameters["id"].orEmpty();val n=Database.connection().use{db->db.prepareStatement("DELETE FROM absences WHERE id=?").use{p->p.setString(1,id);p.executeUpdate()}};if(n==0)call.respond(HttpStatusCode.NotFound,ApiError("NOT_FOUND","Mungesa nuk u gjet."))else call.respond(HttpStatusCode.NoContent)}
 }}
}
...
private fun validateAdminAbsence(r:AdminAbsenceUpdate):String?{if(r.studentId.isBlank())return "Nxënësi është i detyrueshëm.";if(r.subjectId.isBlank())return "Lënda është e detyrueshme.";if(r.teacherId.isBlank())return "Mësimdhënësi është i detyrueshëm.";if(r.date.isBlank())return "Data është e detyrueshme.";if(r.status !in setOf("E_ARSYESHME","E_PAAFTESUAR"))return "Statusi i mungesës nuk është valid.";return null}
private fun loadAdminAbsences():List<AdminAbsenceDto> = Database.connection().use{db->db.prepareStatement("SELECT id,student_id,subject_id,teacher_id,date,status,note FROM absences ORDER BY date DESC,id DESC").use{p->p.executeQuery().use{rs->buildList{while(rs.next())add(AdminAbsenceDto(rs.getString(1),rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(5),rs.getString(6),rs.getString(7)))}}}}