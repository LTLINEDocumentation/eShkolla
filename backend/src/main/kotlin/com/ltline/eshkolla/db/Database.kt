...
object Database {
    ...
    private fun seed(connection: Connection) {
        connection.prepareStatement("INSERT INTO classes(id,name,grade_level) VALUES (?,?,?) ON CONFLICT (id) DO NOTHING").use { ps ->
            listOf("C03" to 7, "C04" to 8).forEach { (id, level) ->
                ps.setString(1, id); ps.setString(2, "Klasa $id"); ps.setInt(3, level); ps.addBatch()
            }
            ps.executeBatch()
        }
        connection.prepareStatement("INSERT INTO users(id,username,full_name,role,password_hash) VALUES (?,?,?,?,?) ON CONFLICT (id) DO NOTHING").use { ps ->
            ps.setString(1, "3"); ps.setString(2, "leonard.tahiraj"); ps.setString(3, "Leonard Tahiraj"); ps.setString(4, "MESIMDHENES"); ps.setString(5, "120000.256.QVt1VQ==.SEED_PLACEHOLDER"); ps.executeUpdate()
        }
        ...
        connection.prepareStatement("INSERT INTO students(id,full_name,class_id,birth_date,active) VALUES (?,?,?,?,?) ON CONFLICT (id) DO NOTHING").use { ps ->
            listOf(
                arrayOf("NX001", "Ardit Krasniqi", "C03", "2012-03-01"),
                arrayOf("NX002", "Era Gashi", "C03", "2012-07-18"),
                arrayOf("NX003", "Diar Berisha", "C04", "2012-02-09"),
                arrayOf("NX004", "Suela Hoxha", "C04", "2012-11-22")
            ).forEach { row ->
