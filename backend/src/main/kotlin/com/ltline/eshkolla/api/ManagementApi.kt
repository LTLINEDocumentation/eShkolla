...
fun Application.configureManagementApi(authService: AuthService) {
    routing {
        route("/api/v1/management") {
            get("/teachers") {
                val user = managementUser(authService) ?: return@get
                ...
                val teachers = Database.connection().use { c ->
                    c.prepareStatement("SELECT t.id,t.full_name,t.subject_id,t.active,u.username FROM teachers t JOIN users u ON u.id=t.user_id ORDER BY t.full_name").use { ps ->
                        ps.executeQuery().use { rs -> buildList {
                            while (rs.next()) {
                                val id = rs.getString("id")
                                add(ManagementTeacher(id, rs.getString("full_name"), rs.getString("subject_id"), rs.getString("username"), rs.getBoolean("active"), classIds(c, id)))
                            }
                        } }
                    }
                }
            ...
            get("/classes") {
                ...
                val classes = Database.connection().use { c ->
                    c.prepareStatement("SELECT id,name,grade_level,active,(SELECT COUNT(*) FROM students s WHERE s.class_id=c.id AND s.active=TRUE) AS student_count FROM classes c ORDER BY grade_level,name").use { ps ->
                        ps.executeQuery().use { rs -> buildList {
            ...
            put("/users/{id}/status") {
                val user = managementUser(authService) ?: return@put
            ...
            post("/teacher-assignments") {
                val user=managementUser(authService) ?: return@post
            ...
            delete("/teacher-assignments/{teacherId}/{classId}") {
                val user=managementUser(authService) ?: return@delete
...
private fun managementUser(auth: AuthService): UserDto? = null
