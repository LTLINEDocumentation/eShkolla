...
class ApiAbsenceRepository : AbsenceRepository {
    ...
    override suspend fun updateAbsence(absence: Absence): Absence = parseAbsence(request("/api/v1/absences/${encode(absence.id)}", "PUT", absenceBody(absence)))
    override suspend fun deleteAbsence(id: String) { request("/api/v1/absences/${encode(id)}", "DELETE") }
